...
void solveGyroscopeBias(map<double, ImageFrame> &all_image_frame, Vector3d* Bgs)
{
    ...
    // 这里使用的代价函数为：
    // F_Q(b_k+1 -> c0)^-1 * F_Q(b_k -> c0) * Pre_Q(b_k+1 -> b_k)
    for (frame_i = all_image_frame.begin(); next(frame_i) != all_image_frame.end(); frame_i++)
    {
        frame_j = next(frame_i);
        MatrixXd tmp_A(3, 3); // jacobian(Q增量误差, ba增量误差)
        tmp_A.setZero();
        VectorXd tmp_b(3);
        tmp_b.setZero();
...
MatrixXd TangentBasis(Vector3d &g0)
{
    ...
    if(a == tmp)
        tmp << 1, 0, 0;
    // 得到准确的向量b1
    // (a.transpose() * tmp) = ||tmp||*cos*||a|| = tmp投影在a上的scalar长度，b垂直于a
...
void RefineGravity(map<double, ImageFrame> &all_image_frame, Vector3d &g, VectorXd &x)
{
    ...
    int n_state = all_frame_count * 3 + 2 + 1;// 重力有两个自由度
    ...
    for(int k = 0; k < 4; k++) // why 4 loop?
    {
        ...
        for (frame_i = all_image_frame.begin(); next(frame_i) != all_image_frame.end(); frame_i++, i++)
        {
            ...
            tmp_A.block<3, 2>(0, 6) = frame_i->second.R.transpose() * dt * dt / 2 
                                      * Matrix3d::Identity() * lxly; // lxly:b(3x2)
            tmp_A.block<3, 1>(0, 8) = frame_i->second.R.transpose() 
                                      * (frame_j->second.T - frame_i->second.T) / 100.0;     
            tmp_b.block<3, 1>(0, 0) = frame_j->second.pre_integration->delta_p // 两帧间P增量 
...
bool LinearAlignment(map<double, ImageFrame> &all_image_frame, Vector3d &g, VectorXd &x)
{
    ...
    for (frame_i = all_image_frame.begin(); next(frame_i) != all_image_frame.end(); frame_i++, i++)
    {
        ...
        double dt = frame_j->second.pre_integration->sum_dt; // 从起始至j帧的累计时间（如何定义其实？IMU?Cam?）

        // TIC: p_b^c: translate poses from camera frame to body (IMU) frame
        tmp_A.block<3, 3>(0, 0) = -dt * Matrix3d::Identity();
        tmp_A.block<3, 3>(0, 6) = frame_i->second.R.transpose() * dt * dt / 2 * Matrix3d::Identity();
        tmp_A.block<3, 1>(0, 9) = frame_i->second.R.transpose() 
                                  * (frame_j->second.T - frame_i->second.T) / 100.0;     
        tmp_b.block<3, 1>(0, 0) = frame_j->second.pre_integration->delta_p // 两帧之间预积分的P增量
                                  + frame_i->second.R.transpose() * frame_j->second.R * TIC[0] - TIC[0];
        //cout << "delta_p   " << frame_j->second.pre_integration->delta_p.transpose() << endl;
        tmp_A.block<3, 3>(3, 0) = -Matrix3d::Identity();
        tmp_A.block<3, 3>(3, 3) = frame_i->second.R.transpose() * frame_j->second.R;
        tmp_A.block<3, 3>(3, 6) = frame_i->second.R.transpose() * dt * Matrix3d::Identity();
        tmp_b.block<3, 1>(3, 0) = frame_j->second.pre_integration->delta_v; // 两帧之间预积分的V增量
...
bool VisualIMUAlignment(map<double, ImageFrame> &all_image_frame, 
                        ...
                        VectorXd &x)
{
    solveGyroscopeBias(all_image_frame, Bgs); // 更新Bgs
