...
class IntegrationBase
{
  public:
    ...
    */
    ...
    void midPointIntegration(double _dt, //time interval(prev->cur IMU data)
                            const Eigen::Vector3d &_acc_0, const Eigen::Vector3d &_gyr_0,
                            const Eigen::Vector3d &_acc_1, const Eigen::Vector3d &_gyr_1,
                            const Eigen::Vector3d    &delta_p, // PVQ增量
                            const Eigen::Quaterniond &delta_q, 
                            const Eigen::Vector3d    &delta_v,
                            const Eigen::Vector3d &linearized_ba, // acc bias增量
                            const Eigen::Vector3d &linearized_bg, // gyro bias增量
                            ...
                            bool update_jacobian)
    {
        ...
        if(update_jacobian)
        {
			...
			// PVQ增量误差的计算： dz(k+1) = F * dz(k) + V * Q
			// F用于计算jacobian： J(k+1) = F * J(k)
			// F,V用于计算covariance: P(k+1) = F * P(k) *FT + V * Q * VT
